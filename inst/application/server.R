...
function(input, output, session) {
  ...
  # Turn an unreadable/missing/malformed file into an informative modal instead
  # of an uncaught error (which, with shiny.sanitize.errors = FALSE, dumps a
  # stack trace into the UI and leaves the tab blank).
  fp_describe_file_problem <- function(path) {
    if (is.null(path) || length(path) != 1 || is.na(path) || !nzchar(path)) {
      return("no file path was resolved for this run (the run may be incomplete)")
    }
    if (!file.exists(path)) return("the file does not exist")
    if (file.access(path, 4) != 0) {
      owner <- tryCatch({
        info <- file.info(path)
        paste0(" (owner: ", info$uname, ", group: ", info$grname,
               ", mode: ", as.character(info$mode), ")")
      }, error = function(e) "")
      return(paste0("you do not have permission to read it", owner))
    }
    NA_character_
  }

  # Read `path` with `reader`, or explain why it could not be read. Returns NULL
  # on failure, having shown the modal.
  fp_read_or_notify <- function(path, reader, what) {
    problem <- fp_describe_file_problem(path)
    if (!is.na(problem)) {
      showModal(modalDialog(
        title = paste0("Cannot open ", what),
        paste0(path, "\n\n", problem), easyClose = TRUE))
      return(NULL)
    }
    tryCatch(reader(path), error = function(e) {
      showModal(modalDialog(
        title = paste0("Cannot read ", what),
  ...
  # Run `expr`, reporting a failure as a modal rather than crashing the observer.
  # Returns NULL on failure. Only ERRORS are caught: warnings are routine here
  # (e.g. metadata_init warns when it cannot write facets_qc.txt on a read-only
  # mount) and must not abort the operation.
  fp_try_or_notify <- function(expr, what, extra = NULL) {
    tryCatch(expr, error = function(e) {
      showModal(modalDialog(
        title = paste0("Failed: ", what),
        paste0(conditionMessage(e), if (!is.null(extra)) paste0("\n\n", extra) else ""),
        easyClose = TRUE))
      NULL
    })
  }

  # Is a directory usable for writing (exists + writable)? Returns NA when fine,
  # else a human-readable reason.
  fp_describe_dir_write_problem <- function(dir) {
    if (is.null(dir) || length(dir) != 1 || is.na(dir) || !nzchar(dir)) {
      return("no directory was configured")
    }
    if (!dir.exists(dir)) return("the directory does not exist")
    if (file.access(dir, 2) != 0) return("you do not have permission to write to it")
    NA_character_
  }
  ...
  cncf_file_for <- function(fit_type, selected_run) {
    if (is.null(selected_run) || nrow(selected_run) == 0) return(NA_character_)
    run_prefix <- if (identical(as.character(fit_type), "Hisens")) {
      selected_run$hisens_run_prefix[1]
    } else {
      selected_run$purity_run_prefix[1]
    }
    if (is.null(run_prefix) || is.na(run_prefix) || !nzchar(run_prefix)) return(NA_character_)
    edited <- paste0(run_prefix, ".cncf.edited.txt")
    if (file.exists(edited)) edited else paste0(run_prefix, ".cncf.txt")
  }

  # get_cncf_table(), but a missing/unreadable/malformed file produces a modal
  # naming the file and the reason instead of an uncaught fread error.
  cncf_table_or_notify <- function(fit_type, selected_run) {
    f <- cncf_file_for(fit_type, selected_run)
    problem <- fp_describe_file_problem(f)
    if (!is.na(problem)) {
      showModal(modalDialog(
        title = "Cannot open the CNCF file",
        paste0(if (is.na(f)) "(no CNCF path could be resolved for this run)" else f,
               "\n\n", problem), easyClose = TRUE))
      return(NULL)
    }
    tryCatch(get_cncf_table(fit_type, selected_run), error = function(e) {
      showModal(modalDialog(
        title = "Cannot read the CNCF file",
        paste0(f, "\n\ncould not be parsed: ", conditionMessage(e)), easyClose = TRUE))
      NULL
    })
  }
  ...
  load_runs_for_pane <- function(pane, sample_id, sample_path, progress = NULL,
    ...
    # metadata_init reads every fit's .out and .Rdata; an unreadable or corrupt
    # one used to abort the whole observer with a raw stack trace. Report it and
    # let the caller's "no runs" handling take over.
    runs <- fp_try_or_notify(
      if (isTRUE(id$is_2n)) {
      ...
      what = "loading FACETS runs",
      extra = paste0("Sample: ", sample_id, "\nPath: ", sample_path,
                     "\n\nCheck that the run directory is readable and its ",
  ...
  observeEvent(input$button_repoSamplesInput, {
    ...
    # load_repo_samples() stop()s on a missing/unreadable/malformed manifest.
    repo_meta <- fp_try_or_notify(
      collapse_manifest_2n(load_repo_samples(tumor_ids, manifest_file, progress)),
      what = "loading the repository manifest",
      extra = paste0("Manifest: ", manifest_file))
  ...
  observeEvent(input$compareFitsCheck, {
    ...
    if (input$compareFitsCheck) {
      ...
      output$datatable_cncf <- DT::renderDataTable({
        cncf_data1 <- cncf_table_or_notify(view_fit_type, selected_run)
        cncf_data2 <- cncf_table_or_notify(view_fit_type_compare, selected_run_compare)

        # cncf_table_or_notify() already explained the failure in its own modal.
        if (is.null(cncf_data1) || is.null(cncf_data2)) { return(NULL) }
      ...
      output$editableSegmentsTable <- rhandsontable::renderRHandsontable({
        cncf_data1 <- cncf_table_or_notify(view_fit_type_compare, selected_run_compare) %>%
          data.frame()
        cncf_data2 <- cncf_table_or_notify(view_fit_type, selected_run) %>%
          data.frame()
      ...
      updateCloseups()
        ...
        output$datatable_cncf <- DT::renderDataTable({
          cncf_data <-
            cncf_table_or_notify(view_fit_type, selected_run)
          # cncf_table_or_notify() already explained the failure in its own modal.
          if (is.null(cncf_data)) { return(NULL) }
...
#  })
  ...
  observeEvent(input$button_addReview, {
    ...
    signed_as <- if (is_vm) fp_user else {
      x <- tryCatch(system("whoami", intern = TRUE), error = function(e) character(0))
      x <- x[nzchar(x)]
      if (length(x) > 0) x[[1]] else ""
    }

    if (!nzchar(signed_as)) {
      showNotification("Unable to determine signed_as (FP_USER_ID missing and whoami failed).", type = "error")
      return(NULL)
    }
  ...
  observeEvent(input$radioGroupButton_fitType, {
    ...
    output$imageOutput_pngImage1 <- renderImage({
      ...
      # Neither the .CNCF.png nor its plain fallback exists: say so rather than
      # rendering a broken image with no explanation.
      problem <- fp_describe_file_problem(png_filename)
      if (!is.na(problem)) {
        showModal(modalDialog(
          title = "Cannot open the FACETS plot",
          paste0(if (is.na(png_filename)) "(no plot path resolved for this run)" else png_filename,
                 "\n\n", problem), easyClose = TRUE))
  ...
  updateCloseups <- function() {
    ...
    if(input$compareFitsCheck)
    {
      output$plotOutput_closeup <- renderPlot ({
        ...
        # A missing/unreadable .Rdata is reported, not thrown.
        for (f in c(rdata_file, rdata_file_compare)) {
          problem <- fp_describe_file_problem(f)
          if (!is.na(problem)) {
            showModal(modalDialog(
              title = "Cannot open the FACETS output (.Rdata)",
              paste0(if (is.na(f)) "(no .Rdata path resolved for this run)" else f,
                     "\n\n", problem), easyClose = TRUE))
            return(NULL)
          }
        }
    ...
    else
    {
      output$plotOutput_closeup <- renderPlot ({
        ...
        problem <- fp_describe_file_problem(rdata_file)
        if (!is.na(problem)) {
          showModal(modalDialog(
            title = "Cannot open the FACETS output (.Rdata)",
            paste0(if (is.na(rdata_file)) "(no .Rdata path resolved for this run)" else rdata_file,
                   "\n\n", problem), easyClose = TRUE))
          return(NULL)
        }
  ...
  observeEvent(input$button_refit, {
    ...
    ## get best fit if exists; otherwise default
    selected_run <- get_selected_run(values$sample_runs)

    # --- Mount/auth logic: skip completely in VM to avoid bad indices ----
    if (!is_vm_mode()) {
      mount_df <- get_mount_info()
      print(selected_run)
      ...
      if (nrow(mount_df) > 0) {
        matched_idx <- vapply(
          mount_df$local_path,
          function(local_path) grepl(local_path, selected_run$path[1]),
          logical(1)
        )
        ...
        if (nrow(matched_row) > 0) {
          if (is_restricted_path(matched_row$remote_path)) {
            if (session_data$password_valid == 1 || !is_remote_file(selected_sample_path)) {
              ...
              showNotification(
                "You are not authorized to perform re-fit on remote sample. Authenticate on the session tab to unlock.",
                type = "error"
              )
              return(NULL)
            }
    ...
    showModal(modalDialog(
      title = "Job submitted!",
      paste0(
        ifelse(refit_note != "", paste("Warning: ", refit_note, "\n\n"), ""),
