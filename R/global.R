...
metadata_init <- function(sample_id, sample_path, progress = NULL, update_qc_file = TRUE) {
  ...
  # Standard runs have no such subdirs -> dir.exists() is FALSE for every fit ->
  # nothing is appended and facets_run_dirs (and all downstream behavior) is unchanged.
...
get_review_status <- function(sample_id, sample_path) {
  ...
  print(review_file)
  # Manifests normally begin with a '# generated by ...' comment line, which fread
  # cannot handle via comment.char (that would also drop '#'-prefixed data). We detect
  # it explicitly and skip it only when present, so a manifest written WITHOUT the
  # comment line still loads instead of crashing -- previously skip=1 would consume the
  # real column header and mangle every column.
  first_line <- tryCatch(readLines(review_file, n = 1, warn = FALSE), error = function(e) character(0))
  skip_n <- if (length(first_line) > 0 && grepl("^\\s*#", first_line[1])) 1 else 0
  reviews <-
    suppressWarnings(fread(review_file, colClasses=list(character="facets_qc_version",
                                       character="facets_suite_version"), verbose = F, skip = skip_n)) %>%
