...
## 0. Push master FIRST — both images clone it from GitHub

`Docker/Dockerfile:58` and the 2n container's `Dockerfile:119` both
`git clone --branch master https://github.com/mskcc/facets-preview.git` at build
time. Only `run_fp.sh` and `fp_config.json` come from the local build context.
...
Delete macOS `._*` files from any COPY'd directory first. The version bump at
`Dockerfile:17` sits above the clone, so the cache below it is invalidated and
the clone genuinely re-runs — no `--no-cache` needed here.

This image finally carries the comment-optional manifest reader
...
### Then pull it on the VM — it will NOT update itself

`facets_app.py` calls `containers.run()` with no pull policy, and docker-py only
pulls when the tag is ABSENT locally. `price0416/fp_docker:dev` already exists on
the VM, so a fresh push to the same tag is silently ignored and sessions keep
...
1. Respin `facets_2n_cadence` 0.1.7 → 0.1.8 and `nf_impact_autoqc_2n` 0.1.0 →
   ...
   detection in `run_autoQC_2n.R`). **Use `--no-cache`**: neither Dockerfile is
   changed by this release, so their `ENV FORCE_REBUILD` lines still hash the
   same and Docker would serve the facets-preview clone layer from cache —
   silently rebuilding the OLD code even after the push. Bump `FORCE_REBUILD`
   or:

   ```bash
   docker buildx build --builder multiarch --no-cache \
     --platform linux/amd64,linux/arm64 \
     -t price0416/facets_2n_cadence:0.1.8 --push .
   ```
